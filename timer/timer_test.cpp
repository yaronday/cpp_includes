...
namespace timer::unittests
{
    ...
    static constexpr bool run_compile_time_tests()
    {
        ...
        // --- Test Case 1: Seconds Boundary (No scaling applied) ---
        {
            ...
            if (!compile_time_approx(value, 1.5) || unit != " [s]")
                return false;
        }
        ...
        // --- Test Case 2: Milliseconds Range ---
        {
            ...
            if (!compile_time_approx(value, 250.0) || unit != " [ms]")
                return false;
        }
        ...
        // --- Test Case 3: Microseconds Range ---
        {
            ...
            if (!compile_time_approx(value, 500.0) || unit != " [us]")
                return false;
        }
        ...
        // --- Test Case 4: Nanoseconds Range ---
        {
            ...
            if (!compile_time_approx(value, 70.0) || unit != " [ns]")
                return false;
        }
        ...
        // --- Test Case 5: Picoseconds Range ---
        {
            ...
            if (!compile_time_approx(value, 10.0) || unit != " [ps]")
                return false;
        }
        ...
        // --- Test Case 6: Edge Case Boundaries ---
        {
            ...
            if (!compile_time_approx(val_ms, 1000.0) || unit_ms != " [us]")
                return false;

            auto [val_zero, unit_zero] = time_scaler(0.0); // Exact zero defaults to picoseconds branch
            if (!compile_time_approx(val_zero, 0.0) || unit_zero != " [ps]")
                return false;
        }
    ...
    static void run_runtime_tests()
    {
        ...
        // --- Test Case 1: Execution Count Precision ---
        {
            ...
            time_it("Count Test", [&]()
                    { call_count++; }, target_iters, true); // printout=true to bypass file logs for testing
